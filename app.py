...
def process_folder(folder_path):
    ...
    return (excel_file_path1, excel_file_path2, numOfFile)
...
if __name__ == "__main__":
    if button:
        st.write(f"Generating dPCR Master Tables for {d4}...")
        st.write("It may take a few mins. Please wait...")
        ...
        num_files = [result[2] for result in results]
        for num_file, folder in zip(num_files, folders):
            st.write(f"{num_file} files found and merged in {folder}")
        ...
        if progress_bar.progress(100):
            st.write("Done! Look for Master tables in the QIAcuity-VIZ-Files folder")
